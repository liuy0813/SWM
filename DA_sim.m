...
for itime = 1 : time
    ...
    if mod(itime,obs_freq) == 0
        ...
        gama = zeros(num_elems) + .03*randn(num_elems);   % Gaussian observation perturbation, Generate values from a normal distribution with mean 0 and standard deviation 1.
        gama = squeeze(gama);
